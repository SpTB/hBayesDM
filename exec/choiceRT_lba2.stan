...
functions{
  ...
  real lba_lpdf(matrix RT, real d, real A, vector v, real s, vector tau) {
  ...
  vector lba_rng(real d, real A, vector v, real s, vector tau) {
...
parameters {
  ...
  real<lower=0> mu_d;
  real<lower=0> mu_A;
  vector<lower=0> [N_choices] mu_tau[N_cond];
  vector<lower=0>[N_choices] mu_v[N_cond];
  
  // Hyperparameter sigmas
  real<lower=0> sigma_d;
  real<lower=0> sigma_A;
  vector<lower=0> [N_choices] sigma_tau[N_cond];
  vector<lower=0>[N_choices] sigma_v[N_cond];
  
  // Individual parameters
  real<lower=0> d[N];
  real<lower=0> A[N];
  vector<lower=0>[N_choices] tau[N,N_cond];
...
model {
  ...
  for(j in 1:N_cond) {
    for(n in 1:N_choices) {
      ...
      sigma_v[j,n] ~ gamma(1,1)T[0,];
    }
  }
  
  // Hyperparameter means and sigmas for multiple T0
  for(j in 1:N_cond) {
    for(n in 1:N_choices) {
      mu_tau[j,n]    ~ normal(.5,.5)T[0,];
      sigma_tau[j,n] ~ gamma(1,1)T[0,];
    }
  }
  ...
  for(i in 1:N) {		
    ...
    for(j in 1:N_cond) {
      ...
      for(n in 1:N_choices){
        ...
        tau[i,j,n] ~ normal(mu_tau[j,n], sigma_tau[j,n])T[0,];
      }
      // Likelihood of RT x Choice
      RT[i,j,,1:n_trials] ~ lba(d[i], A[i], v[i,j,], s, tau[i,j,]);
    }		
  }	
}
  ...
  { // local section, this saves time and space
    for(i in 1:N) {
      ...
      for(j in 1:N_cond) {
        ...
        log_lik[i] = log_lik[i] + lba_lpdf(RT[i,j,,1:n_trials] | d[i], A[i], v[i,j,], s, tau[i,j,]);
        
        for (t in 1:n_trials) {
          // generate posterior predictions
          y_pred[i,j,,t] = lba_rng(d[i], A[i], v[i,j,], s, tau[i,j,]);
        }
      }
    }
    // end of subject loop
